...
describe('shouldRefreshToken', () => {
  ...
  it('does not crash when there is no service at all', () => {
    const user = newUser(now)
    delete user.services.u5auth
    expect(shouldRefreshToken(user, newConfig(10), now)).toBeTruthy()
  })
})